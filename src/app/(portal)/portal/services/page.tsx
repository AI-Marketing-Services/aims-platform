...
// Static demo data — will be replaced with real DB queries once DATABASE_URL is set
const DEMO_SERVICES = [
  {
    id: "svc_1",
...
const statusConfig = {
  active: { icon: CheckCircle, label: "Active", class: "text-green-600 bg-green-50 border-green-200" },
  pending: { icon: Clock, label: "Setting Up", class: "text-yellow-600 bg-yellow-50 border-yellow-200" },
...
export default async function PortalServicesPage() {
  ...
  return (
    <div className="max-w-4xl">
      <div className="mb-8">
        <h1 className="text-2xl font-bold text-white mb-1">My Services</h1>
        <p className="text-gray-400">Active subscriptions and service status</p>
      </div>
      ...
      <div className="grid grid-cols-3 gap-4 mb-8">
        {[
          ...
          <div key={stat.label} className="bg-[#151821] border border-white/10 rounded-xl p-5">
            ...
            <div className="text-2xl font-bold text-white">{stat.value}<span className="text-sm text-gray-400 font-normal ml-1">{stat.sub}</span></div>
          </div>
        ))}
      </div>
      ...
      <div className="space-y-4">
        {DEMO_SERVICES.map((svc) => {
          ...
          return (
            <div
              key={svc.id}
              className="bg-[#151821] border border-white/10 rounded-xl p-6"
            >
      ...
      {/* Upsell CTA */}
      <div className="mt-8 bg-gradient-to-r from-[#DC2626]/20 to-[#DC2626]/5 border border-[#DC2626]/30 rounded-xl p-6 flex items-center justify-between">
        <div>
          <div className="flex items-center gap-2 mb-1">
            <Zap className="w-4 h-4 text-[#DC2626]" />
            <span className="text-white font-semibold">Ready to scale further?</span>
          </div>
          <p className="text-gray-400 text-sm">Browse 15 AI services that plug directly into your existing stack.</p>
        </div>
        <Link
          href="/marketplace"
