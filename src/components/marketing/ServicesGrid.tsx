        ...
        {large.map((service, i) => (
          ...
          >
            <Link
              ...
              className="group flex flex-col sm:flex-row items-start justify-between gap-6 rounded-2xl border border-gray-200 bg-white p-7 shadow-sm hover:border-gray-300 hover:shadow-md transition-all"
            >
              <div className="flex-1">
                ...
                <h3 className="text-xl font-bold text-gray-900 group-hover:text-[#DC2626] transition-colors">
                  {service.name}
                </h3>
                <p className="mt-2 text-sm text-gray-500 leading-relaxed max-w-xl">{service.desc}</p>
                <div className="mt-4 flex flex-wrap gap-2">
                  {service.tags.map((tag) => (
                    <span key={tag} className="rounded-md bg-gray-100 px-2.5 py-0.5 text-xs font-medium text-gray-600">
                      {tag}
                    </span>
                  ))}
                </div>
              ...
              <div className="flex flex-col items-end justify-between self-stretch gap-4 min-w-[180px]">
                <div className="rounded-xl border border-gray-100 bg-gray-50 px-4 py-3 text-right">
                  <p className="text-xs text-gray-400 mb-0.5">Client outcome</p>
                  <p className="text-sm font-bold text-gray-900">{service.outcome}</p>
                </div>
        ...
        <div className="grid grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-3">
          {normal.map((service, i) => (
            ...
            >
              <Link
                href={`/services/${service.slug}`}
                className="group flex flex-col h-full rounded-2xl border border-gray-200 bg-white p-6 shadow-sm hover:border-gray-300 hover:shadow-md transition-all"
              >
                ...
                <h3 className="text-base font-bold text-gray-900 group-hover:text-[#DC2626] transition-colors">
                  {service.name}
                </h3>
                <p className="mt-2 flex-1 text-sm text-gray-500 leading-relaxed">{service.desc}</p>

                <div className="mt-4 rounded-lg bg-gray-50 px-3.5 py-2.5 text-xs text-gray-500">
