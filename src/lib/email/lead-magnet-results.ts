...
export async function sendQuizResultsEmail(params: {
  ...
  const body = `
    ...
    ${divider()}
    ${p("Want the playbooks and operator community to act on these recommendations? The AI Operator Collective is application-only — reviewed within 24 hours.")}
    <a href="https://www.aioperatorcollective.com/apply" style="font-size:14px;color:#981B1B;font-weight:600;text-decoration:none;">
...
export async function sendCalculatorResultsEmail(params: {
  ...
  const body = `
    ...
    ${divider()}
    ${p("Want to implement these savings with operators who've already done it? The AI Operator Collective is application-only — reviewed within 24 hours.")}
    <a href="https://www.aioperatorcollective.com/apply" style="font-size:14px;color:#981B1B;font-weight:600;text-decoration:none;">
      Apply to the AI Operator Collective
    </a>
  `
...
export async function sendAuditResultsEmail(params: {
  ...
  const body = `
    ...
    ${divider()}
    ${p("Want to implement these fixes inside a community of operators doing the same thing? The AI Operator Collective is application-only — reviewed within 24 hours.")}
...
export async function sendCreditScoreEmail({
  ...
  const html = emailLayout(`
    ...
    <div style="background:#F9FAFB;border:1px solid #E5E7EB;border-radius:10px;padding:28px;text-align:center;margin:24px 0;">
      <div style="font-size:64px;font-weight:900;color:#981B1B;line-height:1;">${s}</div>
      <div style="font-size:16px;color:#374151;font-weight:600;margin-top:4px;">out of 100</div>
    ...
    ${divider()}
    ${p(`Want to implement your credit building plan inside a community of operators doing the same thing? The AI Operator Collective is application-only.`)}
    ${btn("Apply to the AI Operator Collective", "https://www.aioperatorcollective.com/apply")}
  `)
...
export async function sendOpsAuditEmail({
  ...
  const costLine = cost > 0
    ? `<div style="background:#FEF2F2;border:1px solid #FECACA;border-radius:10px;padding:20px;text-align:center;margin:16px 0;"><div style="font-size:12px;color:#DC2626;font-weight:700;text-transform:uppercase;letter-spacing:0.06em;margin-bottom:6px;">Estimated Annual Cost of Inefficiency</div><div style="font-size:42px;font-weight:900;color:#DC2626;line-height:1;">$${cost.toLocaleString()}</div></div>`
    : ""

  const roiLine = roi > 0
    ? `<div style="background:#F0FDF4;border:1px solid #BBF7D0;border-radius:10px;padding:20px;text-align:center;margin:16px 0;"><div style="font-size:12px;color:#15803D;font-weight:700;text-transform:uppercase;letter-spacing:0.06em;margin-bottom:6px;">Projected AI ROI Potential</div><div style="font-size:42px;font-weight:900;color:#15803D;line-height:1;">$${roi.toLocaleString()}/yr</div></div>`
    : ""
  ...
  const html = emailLayout(`
    ...
    <div style="background:#F9FAFB;border:1px solid #E5E7EB;border-radius:10px;padding:24px;text-align:center;margin:24px 0;">
      <div style="font-size:56px;font-weight:900;color:#981B1B;line-height:1;">${s}</div>
      <div style="font-size:16px;color:#374151;margin-top:4px;font-weight:600;">Operational Efficiency Score</div>
    ...
    ${divider()}
    ${p(`Want the playbooks, tools, and operator network to actually implement this? The AI Operator Collective is application-only — reviewed by a real operator within 24 hours.`)}
    ${btn("Apply to the AI Operator Collective", "https://www.aioperatorcollective.com/apply")}
  `)
...
export async function sendBusinessAIAuditEmail(params: {
  ...
  const body = `
    ...
    <div style="background:#F9FAFB;border:1px solid #E5E7EB;border-radius:10px;padding:22px 24px;margin:0 0 24px;">
      ...
      <p style="margin:0;font-size:42px;font-weight:800;color:#111827;line-height:1;">${score}<span style="font-size:18px;color:#6B7280;">/100</span></p>
      <p style="margin:8px 0 0;font-size:12px;color:#4B5563;">Higher = more high-leverage AI opportunities we found in your business.</p>
    </div>
...
export async function sendW2PlaybookEmail(params: {
  ...
  const body = `
    ...
    <div style="background:#F9FAFB;border:1px solid #E5E7EB;border-radius:10px;padding:22px 24px;margin:0 0 24px;">
      ...
      <p style="margin:0 0 6px;font-size:15px;color:#374151;line-height:1.6;">Twelve specific plays. Tools, scripts, pricing structures. The same plays AOC operators run before they leave their W-2.</p>
    </div>
